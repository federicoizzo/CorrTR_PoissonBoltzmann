...
if ~false  # ε~2h test on the spheres 
  ...
  @time for i=1:nmax
      ...
      local h = 0.1
      ε = 0.1
      @time hval[i], val_abs[:,:,i], val_err[:,:,i], mval_abs[:,i], mval_err[:,i],  surf_val[:,i], errvec[i,:,:] = PB_gen_shape_system( Nvec[i]; surfTargetXYZ=TPtargets, shift=shift, epslI=1.0, epslE=80.0, kappa_val=0.1257, fε=fε, plotting_surface=(i==1), count=i)
      GC.gc()
  end
  x = detail;
  open(newdir_nrun*"/surf_PB_$(nrun[1])_detail.dat","w") do io
